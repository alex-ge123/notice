...
public class GeTuiServiceImpl {
  ...
  public void pushMsg(MessageDto message, String recipientId, String domain, String clientId) {
    ...
    try {
      ...
      log.debug("ClientId为{}调用第三方(个推)返回状态 ResultCode: {},response:{}", clientId, ipushResult.getResultCode(), ipushResult.getResponse());
      ...
      while (count < ParamConstant.getDEFAULT_REPEAT_COUNT() && ObjectUtil.isNotNull(singleMessage) && ObjectUtil.isNotNull(target)) {
        ...
        if (ObjectUtil.isNotNull(ipushResult)
